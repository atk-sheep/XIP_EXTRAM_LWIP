##########################################################################################################################
# File automatically-generated by tool: [projectgenerator] version: [4.2.0-B44] date: [Thu Sep 26 16:36:37 CST 2024]
##########################################################################################################################
...
# binaries
#######################################
PREFIX = /home/cyyu20/my_ws/AAA_package/arm-gnu-toolchain-13.2.Rel1-x86_64-arm-none-eabi/bin/arm-none-eabi-
...
#######################################
# link script
LDSCRIPT = STM32F767IGTx_QSPIFLASH_EXTSDRAM.ld
...
all: $(BUILD_DIR)/$(RELATIVE_PATH)/$(TARGET).elf $(BUILD_DIR)/$(RELATIVE_PATH)/$(TARGET).hex $(BUILD_DIR)/$(RELATIVE_PATH)/$(TARGET).bin
...
$(BUILD_DIR)/$(RELATIVE_PATH)/$(TARGET).elf: $(OBJECTS) Makefile $(BUILD_DIR) $(RELATIVE_PATH)
	$(CC) $(OBJECTS) $(LDFLAGS) -o $@
	$(SZ) $@

$(BUILD_DIR)/$(RELATIVE_PATH)/%.hex: $(BUILD_DIR)/$(RELATIVE_PATH)/%.elf | $(BUILD_DIR)
	$(HEX) $< $@
	
$(BUILD_DIR)/$(RELATIVE_PATH)/%.bin: $(BUILD_DIR)/$(RELATIVE_PATH)/%.elf | $(BUILD_DIR)
	$(BIN) $< $@	
